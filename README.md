...
Пять MCP-инструментов (`rlm_projects`, `rlm_index`, `rlm_start` → `rlm_execute` → `rlm_end`), песочница с **49 хелперами**, сессионные кэши, таймауты, безопасность, пример построения графа вызовов на крупной конфигурации (23K+ файлов) — **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**
