...
- `read_procedure(path, proc_name, include_overrides=False)` — с `include_overrides=True` дописывает тело расширенного метода с аннотацией и файловой ссылкой. Без параметра — обратная совместимость, чистое тело
